...
<ROUTINE PARSER ("AUX" (PTR ,P-LEXSTART) WORD (VAL 0) (VERB <>)
	...
	<COND (,P-CONT
	       <SET PTR ,P-CONT>
	       <SETG P-CONT <>>
	       <COND (<AND <NOT <VERB? TELL>> <NOT ,SUPER-BRIEF>> <CRLF>)>)
	      (T
	       <SETG WINNER ,ADVENTURER>
...
<ROUTINE ORPHAN-MERGE ("AUX" (CNT -1) TEMP VERB BEG END (ADJ <>) WRD) 
   ...
   <REPEAT ()
	   <COND (<G? <SET CNT <+ .CNT 1>> ,P-ITBLLEN> <RTRUE>)
...
<ROUTINE SYNTAX-CHECK ("AUX" SYN LEN NUM OBJ (DRIVE1 <>) (DRIVE2 <>) PREP VERB TMP) 
	...
	<COND (<AND .DRIVE1
	      ...
	      (T
	       ...
	       <COND (.DRIVE2
		      <TELL " the">
		      <CLAUSE-PRINT ,P-PREP1 ,P-NC1 ,P-NC1L>)>
	       <SETG P-OFLAG T>
...
<ROUTINE CLAUSE-PRINT (PPTR BPTR EPTR "AUX" BEG END) 
	#DECL ((BPTR EPTR PPTR) FIX (BEG END) <PRIMTYPE VECTOR>)
	<SET BEG <GET ,P-ITBL .BPTR>>
	<SET END <GET ,P-ITBL .EPTR>>
	<PREP-PRINT <GET ,P-OTBL .PPTR>>
	<REPEAT ()
		<COND (<==? .BEG .END> <RETURN>)
		      (T
		       <TELL " ">
		       <COND (,P-OFLAG <PRINTB <GET .BEG 0>>)
			     (T
			      <WORD-PRINT <GETB .BEG 2> <GETB .BEG 3>>)>)>
		<SET BEG <REST .BEG ,P-WORDLEN>>>>    
 
<ROUTINE PREP-PRINT (PREP) 
	#DECL ((PREP) FIX)
	<COND (<NOT <0? .PREP>> <TELL " "> <PRINTB <PREP-FIND .PREP>>)>>    
...
<ROUTINE GET-OBJECT (TBL
	...
	<COND (<AND <NOT ,P-NAM>
		    ...
		    <0? ,P-GWIMBIT>>
	       <COND (.VRB
		      <TELL "There is a noun missing in that sentence." CR>)>
	       <RFALSE>)>
	...
	<PROG ()
	      ...
	      <COND ;(<AND <G? .LEN 1> <NOT <0? ,P-GWIMBIT>>> <RFALSE>)
